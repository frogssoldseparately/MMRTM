...
class Tracker extends AddOn {
  ...
  bake() {
    ...
    this.isAnActorizer = Array.from(content.DOM.querySelectorAll("label"))
      .filter((label) => label.innerText.includes("Version"))[0]
      .nextElementSibling.innerText.includes("Actorizer");
  }
...
class CheckFilterer extends AddOn {
  ...
  preWrite() {
    ...
    if (this.getAddOn("tracker").isAnActorizer) {
      ...
      // Remove showHideSettings() updateItemDisplaySettings() and the removableSettings object, as
      // this add-on has its own implementations.
      ...
      // Create and append the settings visibility button.
      const settingsBlock = trackerDOM.querySelector("code");
      const settingsToggle = document.createElement("button");
      // The listener is set in checkFilterer/script
      settingsToggle.type = "button";
      settingsToggle.appendChild(
        document.createTextNode("Toggle Setting Display")
      );
      settingsBlock.after(settingsToggle);
      settingsBlock.className = "invisible";
    }
    this.getAddOn("sharedModifier").oneOffExecute("identifyCodeBlock");
    this.getAddOn("sharedModifier").oneOffExecute("identifyTables");
  }
}
